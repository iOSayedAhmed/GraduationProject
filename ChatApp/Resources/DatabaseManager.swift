    ...
    public func insertUser(with user: ChatAppUser, completion: @escaping (Bool)-> Void) {
        database.child(user.safeEmail).setValue(["first_name": user.firstName , "last_name": user.lastName], withCompletionBlock: { [weak self] error , _ in
            ...
            guard error == nil else {
                print("Faild to wite to database")
                completion(false)
                return
            }
...
extension DatabaseManager {
    ///Create a new conversation with targit user email and first message sent
        ...
        ref.observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            if var conversations = userNode["conversations"] as? [[String: Any]] {
                ...
                ref.setValue(userNode, withCompletionBlock: { [weak self] error, _ in
                    ...
                    self?.finishCraetingConversation(name: name,
                                                     converastionID: conversationId,
                                                     fisrtMessage: firstMessage,
    ...
    private func finishCraetingConversation(name:String,converastionID:String, fisrtMessage:Message, completion: @escaping (Bool)-> Void){
//    {
//        "id": String
...
//        "is_read": true/false,
        ...
        let collectionMessage: [String:Any] = [
            ...
            "id": fisrtMessage.messageId,
            "type":fisrtMessage.kind.messageKidString ,
            "content": message,
            "date": dateString,
        ...
        print("adding conversation \(converastionID)")
        
        database.child("\(converastionID)").setValue(value, withCompletionBlock: {error, _ in
            guard error == nil else {
                completion(false)
                return
            }
    ...
    public func getAllMessagesForConversation(with id: String, completion: @escaping (Result<[Message], Error>)-> Void){
         database.child("\(id)/messages").observe(.value, with: { snapshot in
                   ...
                   let messages: [Message] = value.compactMap({ dictionary in
                    ...
                    if type == "photo" {
                        //photo
                        guard let imageUrl = URL(string: content) , let placeHolder = UIImage(systemName: "photo.fill") else {
                            return nil
                        }
    ...
    ///Sends a message to targit conversation and message
            ...
            strongSelf.database.child("\(conversation)/messages").setValue(currentMessages) { error, _ in
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                    ...
                    strongSelf.database.child("\(currentEmail)/conversations").setValue(databaseEntryConversations, withCompletionBlock: { error, _ in
                        ...
                        // Update the recipient latest message
