    ...
    func listenForMessages(id:String,shouldScrollToBottom: Bool) {
        DatabaseManager.shared.getAllMessagesForConversation(with: id, completion: {[weak self] result in
            ...
            case.failure(let error):
                print("Faild to get All Messages: \(error)")
            }
...
extension ChatViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let image = info[.editedImage] as? UIImage, let imageData = image.pngData() {
            //create unique fileName to photo message
            ...
            StorageManager.shared.uploadMessagePhoto(with: imageData, fileName: fileName, completion: { [weak self ] result in
                ...
                case .success(let urlString):
                    // Ready to send photo message
                    print("Uploading message Photo\(urlString)")
                    guard let url = URL(string: urlString), let placeholder = UIImage(systemName: "photo.fill") else {
                        return
                    }
                ...
                case .failure(let error):
                    print("message photo uploaded error \(error)")
                }
            ...
            StorageManager.shared.uploadMessageVideo(with: videoUrl, fileName: fileName, completion: { [weak self ] result in
                ...
                case .failure(let error):
                    print("message video uploaded error \(error)")
                }
...
extension ChatViewController : MessagesDataSource, MessagesDisplayDelegate, MessagesLayoutDelegate {
    ...
    // configure image message to show it as image
...
extension ChatViewController: MessageCellDelegate {
    ...
    func didTapImage(in cell: MessageCollectionViewCell) {
        ...
        case .photo(let media):
            ...
            let vc = PhotoViewersViewController(url: imageUrl)
            navigationController?.pushViewController(vc, animated: true)
